...
function CreateLecture() {
    ...
    const fetchLectures = async () => {
      ...
      try {
        ...
        console.log("Lectures API Response:", result.data)
        if (result.data && result.data.lectures) {
          dispatch(setLectureData(result.data.lectures))
          toast.success(`Loaded ${result.data.lectures.length} lecture(s)`)
        } else {
          toast.error("No lectures found in response")
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white shadow-xl rounded-xl p-6 mb-6">
          ...
          <div className="flex gap-4">
            ...
            <button className="flex items-center gap-2 px-4 py-2 rounded-md bg-yellow-500 hover:bg-yellow-600 text-white text-sm font-medium" onClick={fetchLectures} disabled={loadingLectures}>
                ...
                <div key={lecture._id || index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 text-sm transition-all"
                    >
                      <FaEdit /> Edit
                    </button>
                  </div>
