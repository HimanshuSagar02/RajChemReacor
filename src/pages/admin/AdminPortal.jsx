...
function AdminPortal() {
  ...
  const fetchPortalStats = async () => {
    ...
    try {
      ...
      console.error("Fetch portal stats error:", error);
                    ...
                    return (
                      <div key={index} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                        <div className="flex items-start gap-4">
                          <div className={`p-3 rounded-lg ${activity.type === 'error' ? 'bg-red-100' : activity.type === 'warning' ? 'bg-yellow-100' : 'bg-blue-100'}`}>
                            <Icon className={`text-xl ${activity.type === 'error' ? 'text-red-600' : activity.type === 'warning' ? 'text-yellow-600' : 'text-blue-600'}`} />
                          </div>
