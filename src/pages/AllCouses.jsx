...
function AllCourses() {
  ...
  const categoryIcons = {
    'Physics': <FaAtom className="text-lg" />,
    'Chemistry': <FaFlask className="text-lg" />,
    'Biology': <FaDna className="text-lg" />,
    'Mathematics': <FaCalculator className="text-lg" />,
    'NEET Preparation': <FaStethoscope className="text-lg" />,
    'JEE Preparation': <FaChalkboardTeacher className="text-lg" />,
    'Board Exam Preparation': <FaAward className="text-lg" />,
    'Others': <FaBook className="text-lg" />
  };
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 pt-20">
      ...
      <button
        ...
        className="fixed top-24 left-4 z-50 bg-black text-[#FFD700] px-4 py-2 rounded-xl md:hidden border-2 border-[#FFD700] shadow-lg flex items-center gap-2 font-semibold"
      >
        <FaFilter /> {isSidebarVisible ? 'Hide' : 'Show'} Filters
      </button>

      {/* Professional Filter Sidebar */}
      <aside className={`w-[320px] h-screen overflow-y-auto bg-gradient-to-b from-black via-gray-900 to-black fixed top-0 left-0 p-6 pt-32 border-r-2 border-[#FFD700] shadow-2xl transition-transform duration-300 z-40 
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-bold text-[#FFD700]">Filter Courses</h2>
          </div>
          <button
            onClick={() => navigate("/")}
            className="text-[#FFD700] hover:text-[#FFC107] transition-colors"
          >
        ...
        <button
          className='w-full px-4 py-3 bg-[#FFD700] text-black font-bold rounded-xl hover:bg-[#FFC107] transition-all shadow-lg flex items-center justify-center gap-2 mb-6'
        ...
        {userData?.role !== "student" && (
          <div className="mb-6 bg-white bg-opacity-10 rounded-xl p-4 border border-[#FFD700] border-opacity-30">
            <label className="font-bold text-[#FFD700] mb-3 flex items-center gap-2">
              <FaGraduationCap /> Filter by Class
            </label>
            <select
              className="w-full border-2 border-[#FFD700] rounded-xl px-4 py-3 bg-black text-white font-semibold focus:outline-none focus:ring-2 focus:ring-[#FFD700]"
              value={selectedClass}
        ...
        {userData?.role === "student" && userData?.class && (
          <div className="mb-6 bg-[#FFD700] bg-opacity-20 border-2 border-[#FFD700] rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
              <FaGraduationCap className="text-[#FFD700] text-xl" />
              <p className="font-bold text-[#FFD700]">Your Class: {userData.class}</p>
            </div>
            <p className="text-white text-sm">Showing courses for your class</p>
          </div>
        )}

        {/* Category Filter */}
        <div className="bg-white bg-opacity-10 rounded-xl p-4 border border-[#FFD700] border-opacity-30">
          <label className="font-bold text-[#FFD700] mb-4 flex items-center gap-2">
          ...
          <div className="space-y-3">
            {[
              ...
              <label
                ...
                className={`flex items-center gap-3 cursor-pointer p-3 rounded-lg transition-all ${
                  category.includes(cat.value)
                    ? 'bg-[#FFD700] bg-opacity-30 border-2 border-[#FFD700]'
              ...
              >
                ...
                />
                <span className="text-[#FFD700] flex items-center gap-2 font-semibold">
                  {cat.icon}
                  {cat.value}
                </span>
              </label>
